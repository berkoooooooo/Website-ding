...
const condoCheck=(sendNotif=false)=>fetch(`https://condogames.xyz/latestupload.json?cache=${Date.now()}`,{headers:{pragma:"no-cache","cache-control":"no-cache"}}).then((response)=>response.json()).then((json)=>{uploadMessage.innerHTML=json.success?"Condo has been found!":json.message
...
condoCheck()
if("Notification"in window&&Notification.permission=="default"&&getSavedData().notify!=false){setTimeout(()=>swal.fire({title:"Auto Uploads",text:"Do you want to receive notifications when a new condo gets uploaded?",icon:"question",showDenyButton:true,confirmButtonText:'Sure',denyButtonText:'Nah...',}).then((result)=>{if(result.isConfirmed){Notification.requestPermission().then((permission)=>{if(permission!="granted")return;new Notification("CondoGames.XYZ",{body:"You will now receive notifications when a new condo gets uploaded!"})
